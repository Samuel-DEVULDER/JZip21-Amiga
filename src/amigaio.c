...
static struct Window* GetWindow(BPTR file)
{
...
fallback:
	...
	if(!con_Win) con_Win = GetWindow(CONSOLE);
	if(con_Win) {
...
static void cleanup( )
{
	if(con_Win && con_Title) SetWindowTitles(con_Win, con_Title, (void*)-1);
	con_Win = NULL;
	if(CONSOLE) {
		exit_console();
